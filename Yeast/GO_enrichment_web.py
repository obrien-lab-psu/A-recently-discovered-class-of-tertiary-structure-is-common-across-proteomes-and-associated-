...
def load_rep_genes():
    ...
    rep_genes = np.loadtxt("DATA/genes_with_representative_proteins.txt", dtype = str, usecols = 0).tolist()
...
def fishers_exact_test(rep_genes: list, GO_class: str):
    ...
    Ent_genes = set(Ent_genes) - set(yeast_knotted_genes) - set(d_ents_genes)
...
def create_excel(rep_genes: list):
    ...
    wb = Workbook()
    wb["Sheet"].title = "Yeast_molecular_function"

    wb.create_sheet("Yeast_biological_process")
    wb.create_sheet("Yeast_cellular_component")
    ...
    wb.save("DATA/Yeast_GO_stats.xlsx")
