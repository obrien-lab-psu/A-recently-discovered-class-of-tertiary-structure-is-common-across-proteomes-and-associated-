...
def get_protein_residues_coordinates(entanglement_info:dict, common_genes:set) -> (dict, dict):

    """
    Capture pdb resids and coordinates of both functional and entanglement genes

    """

    pdb_path = "../SASA/Before_TER_PDBs"
    pdb_resids = {}
    pdb_coor = {}

    for uniprot in common_genes:

        pdb = list(entanglement_info[uniprot].keys())[0]

        chain = list(entanglement_info[uniprot][pdb].keys())[0]

        if uniprot not in pdb_resids.keys():
            pdb_resids[uniprot] = {}

        structure = Universe(f"{pdb_path}/{uniprot}_{pdb}.pdb1", format="PDB").select_atoms(f"segid {chain} and name CA")
        
        pdb_resids[uniprot][pdb] = {}
        pdb_resids[uniprot][pdb][chain] = {}

        alpha_carbons_unique_resids, alpha_carbons_unique_indices = np.unique(structure.resids, return_index=True)

        pdb_resids[uniprot][pdb][chain] = alpha_carbons_unique_resids

        if uniprot not in pdb_coor.keys():
            pdb_coor[uniprot] = {}

        pdb_coor[uniprot][pdb] = {}
        pdb_coor[uniprot][pdb][chain] = {}
        pdb_coor[uniprot][pdb][chain] = structure.positions[alpha_carbons_unique_indices]

    return pdb_resids, pdb_coor
...
def null_dist_p_val_calc(uniprot: str):
    ...
    """
    pdb_resids = np.load("../../pdb_resids_Human_V3.npz", allow_pickle = True)["arr_0"].tolist()
    pdb_coor = np.load("../../pdb_coor_Human_V3.npz", allow_pickle = True)["arr_0"].tolist()
    functional_info = np.load("../../../Mongo_code/human_V2_functional_PDB_free-ligand.npz", allow_pickle = True)["arr_0"].tolist()
    entanglement_info = np.load("../../human_non_covalent_lassos_4_5_no_knots.npz", allow_pickle = True)["arr_0"].tolist()
    ...
    nested_distances = np.load("../distance_matrices/Array_of_distance_matrices.npz", allow_pickle = True)["arr_0"].tolist()

    gene_pdb_chain = f"{uniprot}_{gene_pdb}_{rep_chain}"

    with open("../SASA/human_sasa_dict_new.pkl", "rb") as reader:

        sasa_data_arrays = pickle.load(reader)[uniprot][gene_pdb][rep_chain]

    for iteration in range(50_000):

        print(uniprot, iteration)
    ...
    if ent_res is not None:
        ...
        directory = "individual_p_vals_mc_stochastic_0.01"

        func_site = "PDB_free-ligand"
...
def main():
    ...
    functional_sites = ["PDB_free-ligand"]
    ...
    for func_site in functional_sites:
        ...
        if functional_info and entanglement_info:
            ...
            done_genes = set([pval_file.split("-") [0] for pval_file in os.listdir(f"individual_p_vals_mc_stochastic_0.01/{saved_func_site}")])

            skipped = {"P16615", "Q8NB78", "O60551", "P30419", "Q9H0W9", "Q14562", "P11413", 
            "Q16790", "P14735", "O43570", "Q96HY7", "P04040", "Q9BZE1", "Q9NP92", "Q2NL82", 
            "Q9NR71", "Q8N1Q1", "P43166", "P27338", "Q9BU02", "P35219", "Q9UGM6", 
            "Q96PN6", "P21399", "Q06278", "P21397", "P16278", "Q92542"}

            common_genes = common_genes - done_genes - skipped 

            print(len(common_genes), len(done_genes))

            pdb_resids, pdb_coor = get_protein_residues_coordinates(entanglement_info, common_genes)
...
if __name__ == "__main__":
    ...
    # directories = {"individual_p_vals_mc_stochastic_0.01", 
    #                 "individual_p_vals_mc_stochastic_0.01/PDB_DNA_binding", 
    #                 "individual_p_vals_mc_stochastic_0.01/PDB_free_ligand", 
    #                 "individual_p_vals_mc_stochastic_0.01/PDB_metal_interface",
    #                 "individual_p_vals_mc_stochastic_0.01/protein", 
    #                 "individual_p_vals_mc_stochastic_0.01/PDB_RNA_binding", 
    #                 "individual_p_vals_mc_stochastic_0.01/zinc_finger_region", 
    #                 "individual_p_vals_mc_stochastic_0.01/active_site"}
