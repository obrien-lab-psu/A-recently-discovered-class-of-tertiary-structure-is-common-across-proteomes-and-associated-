...
logging.basicConfig(filename="human_structural_analysis.log", format="%(asctime)s %(message)s", filemode="w")
...
def null_dist_p_val_calc(uniprot: str):
    ...
    if ent_res is not None:
        ...
        with open(f"DATA/human_mc_pvals_stochastic_full/{uniprot}-PW-Pval.json", "w") as js:
...
def main():
    ...
    if functional_info and entanglement_info:
        ...
        done_genes = set([p_value_files.split("-")[0] for p_value_files in os.listdir("DATA/human_mc_pvals_stochastic_full")])
...
if __name__ == "__main__":
    ...
    # directories = {"DATA/human_mc_pvals_stochastic_full"}
