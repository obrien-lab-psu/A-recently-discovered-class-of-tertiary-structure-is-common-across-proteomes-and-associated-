...
def null_dist_p_val_calc(param: tuple):
    ...
    pdb_resids = param[0]
    pdb_coor = param[1]
    functional_info = param[2]
    entanglement_info = param[3]
    func_site = param[4]
    common_genes = param[5]
...
def main():
    ...
    for func_site in functional_sites:
        ...
        if not func_site.startswith("PDB_metal_interface"):
            ...
            functional_info = np.load(f"DATA/ecoli_V5_functional_{func_site}.npz", allow_pickle = True)["arr_0"].tolist()

        else:

            functional_info = np.load("DATA/PDB_metal_interface_mc.npz", allow_pickle=True)["arr_0"].tolist()
        ...
        if functional_info and entanglement_info:
            ...
            done_genes = [pval_file.split("-") [0] for pval_file in os.listdir(f"DATA/individual_p_vals/{saved_func_site}")]
            ...
            with mp.get_context("spawn").Pool(cores) as p:
                ...
                results = p.apply_async(null_dist_p_val_calc, args=(params,))
